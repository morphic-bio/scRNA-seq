...
class OWGather_barcode_filters(OWBwBWidget):
    name = "Gather_barcode_filters"
    description = "alpine bash with wget curl gzip bzip2"
    priority = 1
    icon = getIconName(__file__,"bash.png")
    want_main_area = False
    docker_image_name = "biodepot/gather_barcode_filters"
    ...
    optionsChecked=pset({})
    genebarcodedir=pset(None)
    genefeaturedir=pset(None)
    aligndir=pset(None)
    filterfile=pset(None)
    aligner=pset(None)
    def __init__(self):
        super().__init__(self.docker_image_name, self.docker_image_tag)
        with open(getJsonName(__file__,"Gather_barcode_filters")) as f:
